...
export function AssetPortfolioSection({
  ...
  const requestedMonth = snapshotView?.selectedMonth ?? null;
  const selectedSnapshotMonth =
    requestedMonth &&
    monthlyTrendRows.some((row) => row.month === requestedMonth)
      ? requestedMonth
      : latestSnapshotMonth;
...
function AssetTypeBreakdown({ values }: { values: ManualProfileValues }) {
  ...
  return (
    <section
      aria-labelledby="snapshot-asset-breakdown-heading"
      className={reviewSubtlePanelClass("p-3")}
      data-testid="snapshot-asset-breakdown"
    >
      <h3
        className="text-sm font-semibold text-seed-950"
        id="snapshot-asset-breakdown-heading"
      >
        Assets by type
      </h3>
